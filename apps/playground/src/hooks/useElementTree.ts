...
import { CoralElementType, CoralNode, ResponsiveStyle as CoreResponsiveStyle } from '@reallygoodwork/coral-core'
...
export const useElementTree = () => {
  ...
  const addElement = useCallback(
    (elementType: CoralElementType, parentId?: string, name?: string) => {
  ...
  const removeElement = useCallback(
    (elementId: string) => {
      ...
      const updated = elements
        .filter((el) => !toRemove.has(el.id))
        .map((el) => ({
          ...el,
          children:
            el.children?.filter((child) =>
              typeof child === 'object' && 'id' in child ? !toRemove.has((child as ElementTreeNode).id) : true,
            ) || [],
        }))
  ...
  const updateElement = useCallback(
    (elementId: string, updates: Partial<ElementTreeNode>) => {
      ...
      const updated = elements.map((el) => (el.id === elementId ? { ...el, ...updates } : el))
  ...
  const moveElement = useCallback(
    (elementId: string, newParentId?: string, index?: number) => {
      ...
      let updated = elements.map((el) => (el.id === elementId ? { ...el, parentId: newParentId } : el))
      ...
      if (index !== undefined) {
        ...
        console.log(
          'Updated elements:',
          updated.map((e) => ({ id: e.id, name: e.name, orderIndex: e.orderIndex, parentId: e.parentId })),
        )
      }
