...
export const ElementProperties = ({ element, onUpdateElement }: ElementPropertiesProps) => {
  ...
  const handleAddBreakpoint = (breakpoint: Omit<Breakpoint, 'id'>) => {
    const newBreakpoint: ResponsiveStyle = {
      id: `breakpoint_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
            ...
            {activeBreakpointId ? (
              <>
                ...
                <Badge variant="secondary" className="ml-1">
                  {breakpoints.find((b) => b.id === activeBreakpointId)?.label ||
                    breakpoints.find((b) => b.id === activeBreakpointId)?.value}
                </Badge>
              </>
            ) : (
              'Base Element Styles'
