...
/**
 * Converts style value to Vue inline style value
 */
function styleValueToVue(value: unknown): string {
...
// Note: Vue inline styles use camelCase, not kebab-case
// So we keep the property names as-is

/**
 * Converts Coral style object to Vue inline style object string
 * @param styles - Coral style object
 * @returns Vue inline style object string (e.g., "{ color: 'red', padding: '10px' }")
 */
export function stylesToInlineStyle(styles?: CoralStyleType): string {
  ...
  for (const [key, value] of Object.entries(styles)) {
    ...
    // Vue inline styles use camelCase property names
    const cssValue = styleValueToVue(value)
